...
rdfMatriceInertie <- function(img) {
  u20 <- rdfMomentCentre(img, 2, 0)
  u11 <- rdfMomentCentre(img, 1, 1)
  u02 <- rdfMomentCentre(img, 0, 2)
  matrice <- matrix(c(u20, u11, u11, u02), nrow=2, ncol=2, byrow=TRUE)
}

# Valeurs propres et vecteurs propres associés
rdfValVectPropres <- function(matrice) {
  res <- eigen(matrice)
}
